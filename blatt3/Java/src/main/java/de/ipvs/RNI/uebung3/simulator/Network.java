...
import de.ipvs.RNI.uebung3.solution.DVNode;
import de.ipvs.RNI.uebung3.solution.ExampleNode;
import de.ipvs.RNI.uebung3.solution.LSRNode;

/**
 * A class representing an entire network to be simulated. Since everything here is static, these
 * are essentially scoped globals. You may only run one network at a time.
 */
public class Network {
     ...
     * 
     * @param argv
     *            Command-line arguments. argv[0] must be the name of a .net file to read.
     */
    public static void runNetwork(String alg, String topologyFile) {
        nodesByName = new Hashtable<String, Node>();
        linksByName = new Hashtable<String, Link>();
        nodes = new Vector<Node>();
        nodeCoords = new Vector<FPoint>();
        links = new Vector<Link>();
        nodeNames = new Vector<String>();
        now = 0.0;
        eventQueue = new EventQueue();

        readTopology(topologyFile, alg);
    ...
    public static void sendPacket(int source, int dest, String pkt) {
        ...
        PacketEvent pe = new PacketEvent(now + l.timeToTransmit(pkt.length()), source, dest, pkt);
        try {
            eventQueue.insert(pe);
     ...
     * 
     * @param filename
     *            The name of the file, typically ending in ".net"
     */
    private static void readTopology(String filename, String alg) {
        int line = 0;
        BufferedReader in;
        try {
            ...
            while ((s = in.readLine()) != null) {
                ...
                try {
                    if (command.equalsIgnoreCase("node")) {
                        ...
                        System.out.println(filename + ":" + line + ": unknown command: " + command);
                    }
                } catch (NoSuchElementException e) {
                    System.out.println(filename + ":" + line + ": missing argument(s) to "
                            + command);
    ...
    /**
     * Process a "node" command. This function creates a new node based on the information it
     * parses.
     * 
     * @param t
     *            A string tokenizer object representing the line from the .net file with the first
     *            token (the command) already removed.
     * @exception java.lang.NumberFormatException
     *                Thrown when a numeric field (i.e., bandwidth or latency) contains an invalid
     *                number.
     */
    private static void cmdNode(StringTokenizer t, String alg) throws NumberFormatException {
        ...
        switch(alg) {
        case "Example": 
            newNode = new ExampleNode(nodes.size());
            break;
        case "DV": 
            newNode = new DVNode(nodes.size());
            break;
        case "LSR": 
            newNode = new LSRNode(nodes.size());
            break;
        default:
            System.err.println("Ungültiger Algorithmus, nur Example, DV und LSR erlaubt!");
            System.exit(1);
        }
    ...
    /**
     * Process a "link" command. This function creates a new link based on the information it
     * parses.
     * 
     * @param t
     *            A string tokenizer object representing the line from the .net file with the first
     ...
     *                Thrown when a link description references a node that cannot be found in the
     *                nodesByName mapping.
     * @exception java.lang.NumberFormatException
     *                Thrown when a numeric field (i.e., bandwidth or latency) contains an invalid
     *                number.
     */
    private static void cmdLink(StringTokenizer t) throws NoSuchNodeException,
        ...
        Link newLink = new Link(firstNode.getAddress(), secondNode.getAddress(), speed, latency,
                cost);
    ...
    /**
     * Process an "event" command. This function creates and queues a new event based on the
     * information it parses.
     * 
     * @param t
     *            A string tokenizer object representing the line from the .net file with the first
     *            token (the command) already removed.
     * @exception NoSuchLinkException
     *                Thrown when an event description references a link that cannot be found in the
     *                linksByName mapping.
     */
    private static void cmdEvent(StringTokenizer t) throws NoSuchLinkException {
        ...
        if (eventType.equalsIgnoreCase("state")) {
            ...
            LinkEvent le = new LinkEvent(Double.valueOf(eventTime).doubleValue(), link,
                    linkState.equalsIgnoreCase("up"));
            try {
                eventQueue.insert(le);
            } catch (TimeInconsistencyException e) {
                System.out.println("Internal error: event occurs in the past.");
            }
        } else
    ...
    /**
     * Bring a given link up or down. This function takes care of calling Up() or Down() for all
     * appropriate interfaces and calling interfaceUp() or interfaceDown() for all appropriate
     * nodes.
     * 
     * @param link
     *            The link to bring up or down.
     * @param up
    ...
    /**
     * Actually run the simulation -- pop events off the queue and run them until no events remain.
     */
    private static void simulate() {
        ...
        while (!eventQueue.empty()) {
            ...
            System.out.println(">> Time = " + now + "  packetsSent = " + packetsSent);
            // System.out.println("Event at time " + now + ": " + e);
            e.run();
            while (!eventQueue.empty()
                    && (eventQueue.peekNext().getTime() == now || eventQueue.peekNext().getTime() < stopTime)) {
    ...
    /**
     * Dump the current state of the simulator into the output frame. This involves updating the
     * state of the routing table view (rtview) and the network map view (nmview) with new link
     * state and routing table information.
     */
    private static void dump() {
        for (int i = 0; i < nodes.size(); i++) {
            Node n = (Node) nodes.elementAt(i);
            String table = "dest  next  cost\n";
            Enumeration<Route> e = n.getRoutingTable();
            while (e.hasMoreElements()) {
                Route r = (Route) e.nextElement();
                table += "  " + r.getDest() + "     " + r.getNextHop() + "     " + r.getCost()
                        + "\n";
            }
    ...
    /**
     * Finds a link object given two endpoints. If the data structures are consistent, the link
     * returned should be unique.
     * 
     * @param a
     *            The address of one endpoint.
     * @param b
     *            The address of the other endpoint.
     */
    private static Link findLinkByNodes(int a, int b) {
        for (int i = 0; i < links.size(); i++) {
            if ((((Link) links.elementAt(i)).getEndpointA() == a && ((Link) links.elementAt(i))
                    .getEndpointB() == b)
                    || (((Link) links.elementAt(i)).getEndpointA() == b && ((Link) links
                            .elementAt(i)).getEndpointB() == a))
    ...
    private static Vector<Link> links;
    /** The coordinates (X and Y in the range 0.0-1.0) of each node. */
    private static Vector<FPoint> nodeCoords;
    /** A mapping from node names (in the .net file) to node objects. */
    private static Hashtable<String, Node> nodesByName;
    /** A mapping from link names (in the .net file) to link objects. */
    private static Hashtable<String, Link> linksByName;
    /** Each node's name (in the .net file) */
    private static Vector<String> nodeNames;
    /**
     * The simulator's current time (the time of the next event in the queue).
     */
