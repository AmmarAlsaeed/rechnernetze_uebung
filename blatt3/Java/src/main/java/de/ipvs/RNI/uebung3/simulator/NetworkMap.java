...
/**
 * A GUI widget that displays a network map. The display consists of one square for each node, with
 * the node's name, and a line for each link that is up. See the documentation for the base class,
 * Component, for descriptions of most of the functions.
 * 
 * @see Component
 */
public class NetworkMap extends Component {
    ...
    /**
     * The margin, in pixels, between the edge of the map and the edge of the widget.
     */
        ...
        addComponentListener(new ComponentAdapter() {
            public void componentResized(ComponentEvent e) {
                ...
                for (int i = 0; i < nodes.length; i++) {
                    coords[i] = new Point((int) (fcoords[i].x * (width - margin) + margin / 2),
    ...
    public Dimension getPreferredSize() {
        return new Dimension(2 * margin + 600, 2 * margin + 450);
    }
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < links.length; i++)
            if (links[i].isUp()) {
                ...
                g.clearRect(midx - width / 2 - textMargin, midy - ascent - textMargin, width + 2
                        * textMargin, ascent + descent + 2 * textMargin);
                g.drawString(cost, midx - width / 2, midy);
            }
        for (int i = 0; i < nodes.length; i++) {
            int width = fm.stringWidth(nodeName[i]);
            g.clearRect(coords[i].x - width / 2 - textMargin, coords[i].y - ascent - textMargin,
                    width + 2 * textMargin, ascent + descent + 2 * textMargin);
            g.drawRect(coords[i].x - width / 2 - textMargin, coords[i].y - ascent - textMargin,
                    width + 2 * textMargin, ascent + descent + 2 * textMargin);
     ...
     * 
     * @param node
     *            The number of the node to set.
     * @param p
     *            Where to draw the node on the screen. Coordinates are in the range 0.0-1.0 rather
     *            than literal screen coordinates. (0.0, 0.0) is still the upper-left corner.
     */
    public void setLocation(int node, FPoint p) {
        fcoords[node] = p;
        coords[node] = new Point((int) (fcoords[node].x * (getSize().width - margin) + margin / 2),
     ...
     *            The number of the node to set.
     * @param name
     *            The string to set the node's name to.
     */
    ...
    /**
     * The literal coordinates at which to draw each node, calculated automatically from fcoords,
     * above.
     */
