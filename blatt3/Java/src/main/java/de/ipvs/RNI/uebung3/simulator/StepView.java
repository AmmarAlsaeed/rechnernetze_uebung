...
/**
 * A GUI widget to control the simulation. The display consists of the current time (the time of the
 * next event), a Step button, and a Quit button.
 */
public class StepView extends Frame {
    /**
     * Create a new StepView widget.
     * 
     * @param simulatorMonitor
     *            The object which the simulator (the Network object) is wait()ing on. We need this
     *            here so that we can notify() it every time the user hits the Step button.
     */
        ...
        stepTil.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ev) {
                try {
                    ...
                    synchronized (simulatorMonitor) {
                        simulatorMonitor.notify();
                    }
     ...
     * 
     * @param contents
     *            What to put in the time display, typically a number, sometimes with the word
     *            "(done)".
     */
