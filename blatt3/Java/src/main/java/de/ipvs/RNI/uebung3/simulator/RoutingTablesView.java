...
/**
 * A GUI widget that displays the routing table for each node. The display consists of one
 * scrollable text widget for each node, containing an arbitrary string (typically a table of the
 * form &lt;destination,nextHop,cost&gt;) for each node.
 */
public class RoutingTablesView extends Frame {
    /**
     * Create a new routing tables container with the given number of nodes.
     * 
     * @param numNodes
    ...
    public RoutingTablesView(int numNodes) {
        ...
        while (Math.ceil(numNodes * 1.0 / numNodesAcross) == Math.ceil(numNodes * 1.0
                / (numNodesAcross - 1)))
            numNodesAcross--;
        ...
        for (int i = 0; i < numNodes; i++) {
            tables[i] = new TextArea("", numNodes + 2, 30, TextArea.SCROLLBARS_NONE);
        ...
        setTitle("Routing Simulator: Routing Tables for " + this.numNodes + " nodes");
        pack();
    }
     ...
     * 
     * @param table
     *            Which routing table to set (node number).
     * @param contents
     *            What to set the table to.
     */
    public void setTable(int table, String contents) {
        tables[table].setText(contents);
        // next line was added due to strange graphic behavior...
